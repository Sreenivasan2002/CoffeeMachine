...
def is_sufficient(order_incrediants):
    ...
    for items in order_incrediants:
        if order_incrediants[items] > resources["ingredients"][items]:
...
def process_coins(drink, choice):
    ...
    print(round(f"Amount paid: ${total}"),2)
    price = drink["cost"]
...
def deduct_ingrediants(resources,drink):
    for items in resources:
        resources[items]=resources[items]-drink[items]
...
def coffee_machine():
    ...
    while is_on:
        ...
        if choice == "off":
            ...
            for items in resources:
                resources[items] = resources[items] - drink["ingredients"][items]

            # deduct_ingrediants(resources, drink)
